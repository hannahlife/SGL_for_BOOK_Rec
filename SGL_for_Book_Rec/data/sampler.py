...
#修改函数：对于多交互用户仍然pop采样，对于只有一条交互记录的用户，随机采样，即每次都选同一个 item
@typeassert(user_pos_dict=OrderedDict, num_samples=int, num_item=int)
def _pairwise_sampling_v2(user_pos_dict, num_samples, num_item):
    if not isinstance(user_pos_dict, dict):
        raise TypeError("'user_pos_dict' must be a dict.")
    ...
    for user, pos_len in user_pos_len.items():
        try:
            ...
            if len(pos_items) == 1:
                # 每次都选同一个item
                user_pos_sample[user] = [pos_items[0]] * pos_len
            else:
                # 对于多交互用户，使用pop方法采样
                # 先随机打乱正样本列表
                shuffled_pos = list(pos_items.copy())
                np.random.shuffle(shuffled_pos)
                # 如果需要的样本数大于可用正样本数，则循环使用
                if pos_len > len(shuffled_pos):
                    # 计算需要重复多少次
                    repeat_times = (pos_len + len(shuffled_pos) - 1) // len(shuffled_pos)
                    shuffled_pos = (shuffled_pos * repeat_times)[:pos_len]
                user_pos_sample[user] = shuffled_pos

            # 负样本采样（保持与原代码一致）
            neg_items = randint_choice(num_item, size=pos_len, replace=True, exclusion=user_pos_dict[user])
            # 确保返回列表
            if isinstance(neg_items, Iterable):
                user_neg_sample[user] = list(neg_items)
            else:
                user_neg_sample[user] = [neg_items]
                
        except Exception as e:
            print(f'[Warning] Sampling failed for user {user}: {e}')
            user_pos_sample[user] = [0] * pos_len
            user_neg_sample[user] = [0] * pos_len
